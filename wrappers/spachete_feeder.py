...
MACHETE = os.path.join(ROOT_DIR,'MACHETE')
SPORK = os.path.join(ROOT_DIR,'SPORK')
WRAPPERS = os.path.join(ROOT_DIR,'wrappers')
...
with open(FullStemFile,"r") as stem_file:
    ...
    for ind,stem in enumerate(sorted(stems)):
        ...
        if not use_SLURM:
            processes.append(subprocess.Popen(["python",os.path.join(WRAPPERS,"spachete_run.py"),
                                               "--root-dir",ROOT_DIR,
